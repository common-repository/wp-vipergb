...
function vgb_admin_page()
{
  ...
  $vgb_admin_nonce = 'vgb_admin_nonce';
  ?>
  <div class="wrap">
    <?php
      if (isset($_POST['opts_updated'])) {
        ...
        if (!isset($_REQUEST[$vgb_admin_nonce])) {
          die('Error: Missing nonce');
        }
        if(wp_verify_nonce($_REQUEST[$vgb_admin_nonce], $vgb_admin_nonce) != 1)
        {
        ...
        ?><div class="updated">
        <p><strong><?php _e('Options saved.', 'wp-vipergb'); ?></strong></p>
      </div><?php
                if (get_option($opt_vgb_page) != 0 && !comments_open(get_option($opt_vgb_page))) {
                  ...
                  ?><div class="error">
          <p><strong><?php _e('Notice: Comments were disabled for the Guestbook Page you selected.  They\'ve now been automatically enabled, as required for your guestbook to function.', 'wp-vipergb'); ?></strong></p>
        </div><?php
                  }
                }
    ...
    <form name="formOptions" method="post" action="">
      ...
      <?php wp_nonce_field($vgb_admin_nonce, $vgb_admin_nonce) ?>
      <div class="submit"><input type="submit" name="Submit" class="button button-primary" value="<?php _e('Save', 'wp-vipergb') ?>" /></div>
    </form>

    <hr />
    <h4><?php _e('Development', 'wp-vipergb') ?></h4>
    <?php _e("Many hours have gone into developing & maintaining this plugin, far beyond my own personal needs. Although I offer it to you freely, please keep in mind that each hour spent on it was an hour that could've also gone towards income-generating work. If you find it useful, a donation would be greatly appreciated.", 'wp-vipergb') ?><br /><br />
    ...
    <div class="donateButton" style="float:left;text-align:center;width:200px;">
      ...
      <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
        ...
        <input type="hidden" name="return" value="https://www.justin-klein.com/thank-you" />
