 ...
 * Plugin URI: https://www.justin-klein.com/projects/wp-vipergb
...
/*
 * Copyright 2010-2024 Justin Klein (www.justin-klein.com)
 *
...
$vgb_name               = "WP-ViperGB";
$vgb_homepage           = "https://www.justin-klein.com/projects/wp-vipergb";
$vgb_version            = "1.6.2";
...
function vgb_replace_content($content)
{
    ...
    global $opt_vgb_show_browsers, $opt_vgb_show_flags, $opt_vgb_show_cred_link;
    return vgb_GetGuestbook(array(
        'entriesPerPg' => get_option($opt_vgb_items_per_pg),
...
//For block-based themes (i.e. Twenty Twenty-Three), the comments_template filter no longer works.
//Instead, this is how we suppress the default comments "block."
add_filter('pre_render_block', function($pre_render, $parsed_block){
    global $post, $opt_vgb_page;
    if($post && $post->ID == get_option($opt_vgb_page) && $parsed_block['blockName'] == "core/comments"){
        return "";
    }
}, 10, 2);
...
{
    $func = function($matches){return "<img src=\"$matches[1]\" class=\"ecu_images\" alt=\"" . basename($matches["1"]) . "\" />";};
    return preg_replace_callback('/\[img=?\]*(.*?)(\[\/img)?\]/', $func, $content);
}
